...
Experimentation is the backbone of discovery — but **experimentation ≠ optimization**. Most companies (and most growth/"experimentation" teams) only run **low-risk optimization** experiments (A/B funnel tweaks — *value capture*) and mistake that for discovery. Real discovery also runs **high-risk, high-reward** experiments — *value creation* — that get you off the local maximum ([[2024-06-24-cagan-pm-and-experimentation-testing-insights|canon]]). Amazon Prime came from big, hard experiments (mostly backend/shipping), not funnel tests. Corollaries:
