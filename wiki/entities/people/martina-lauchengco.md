---
title: Martina Lauchengco
type: person
aliases: [martina-lauchengco, lauchengco]
status: drafting
tags: [person, svpg, partner, product-marketing]
role: Partner, Silicon Valley Product Group; author of LOVED
org: "[[svpg]]"
related: ["[[svpg]]", "[[marty-cagan]]", "[[christian-idiodi]]", "[[loved]]", "[[product-marketing]]", "[[pmm-ambassador]]", "[[pmm-strategist]]", "[[pmm-storyteller]]", "[[pmm-evangelist]]", "[[transformed]]", "[[feedback]]"]
created: 2026-07-04
updated: 2026-09-05
---

# Martina Lauchengco

> SVPG partner who leads the group's **[[product-marketing|product marketing]]** practice and wrote *[[loved|LOVED]]* (2022) — the product-marketing volume of the SVPG series ([[inspired]] · [[empowered]] · *LOVED* · [[transformed]]). Teaches product marketing at UC Berkeley; earlier led marketing/product on Microsoft Office and Netscape Communicator.

## Role here
The wiki's canon on **product marketing as a distinct, load-bearing competency** — the *market* side of product-market fit — traces to Lauchengco. Cagan repeatedly defers to her on the topic: in the [[2022-03-07-cagan-product-ops-dan-olsen|Product Ops fireside]] he notes "my partner Martina" has "a book coming out… that takes a stab at what great product marketing is," and points to product marketing (not product ops) as the role most product managers actually need a partner for, especially with a direct sales channel.

## Career origins (via *LOVED*)
Lauchengco's LOVED opens by grounding her PMM point of view in specific worked-experience moments — useful when coaching a PM who wonders how a PMM's judgment is earned:

- **Microsoft Word (early 1990s)** — was on the product-marketing team when Windows-release timing forced a half-cycle product schedule; the team pivoted to positioning the smaller release around *"what matters most for how most people use Word"* (grounded in an instrumentation study of keystrokes). Walt Mossberg's review — normally famously unmovable — surprised the team: *"the best writing tool out there."* That version became Word's best-reviewed and most successful up to that time. Grounds Ch 2 (the four fundamentals) and Ch 6 (evangelism activation for reviewers/pundits).
- **Netscape Communicator** — later PMM work Lauchengco references in *LOVED* framing.
- **UC Berkeley** — teaches product marketing.
- **SVPG partner** — leads the group's product-marketing practice.

She's profiled by name in *[[inspired|INSPIRED]]* (Ch 40 — *Profile: Martina Lauchengco of Microsoft*) as one of Cagan's six PMs illustrating the role.

## *Product Therapy* podcast — co-host with [[christian-idiodi|Idiodi]] (2024–)
Lauchengco appears on 5 episodes of the *Product Therapy* podcast (Batch 1 · Eps 7, 10; Batch 2 · Eps 16, 19; Batch 3 · Ep 41):

**Batch 1:**
- [[2024-08-29-idiodi-lauchengco-coaching-feedback]] — Ep 7; the SHARED-style three-step feedback frame (context → your experience → ask their experience); the two phrases *"I'm curious if…"* and *"have we considered…"*; the *"email is the worst place on the planet for feedback"* rule; feedback-as-coaching-in-a-moment. Primary source for [[feedback]].
- [[2024-11-21-idiodi-lauchengco-coaching-product-marketing]] — Ep 10; the podcast-form restatement of *LOVED*'s thesis; the wiki's canonical **Microsoft AI market-shaping case**; the *"average PMM checklist"* anti-pattern. Enrichment source for [[product-marketing]].

**Batch 2:**
- [[2025-03-06-idiodi-lauchengco-coaching-startups]] — Ep 16; the wiki's canonical teaching on **product in startups**. Ideas are dime-a-dozen; **market-product fit** (not the reverse); the 9–10x adoption bar (Andreessen/Horowitz, cited); great-founder traits — *"do they run into breaches when they see a gap?"*; co-founder rule 100%; opportunistic first-hire (Davey Crockett rule); *"invest in the people, not the idea."* Primary root for [[product-in-startups]].
- [[2025-05-01-idiodi-lauchengco-coaching-decision-making]] — Ep 19; the wiki's canonical teaching on team-and-PM-level **decision making**. Garbage-can theory of decisions (relative to point in time); high-integrity decisions; **data-informed vs data-driven**; the *"2s tell you what's broken; the 10s just pat you on the back"* rule; gut feel as informed intuition; Amazon one-way/two-way doors; never delegate to AI. Primary root for [[decision-making]].

**Batch 3:**
- [[2026-06-18-idiodi-lauchengco-product-marketing-in-the-ai-era]] — Ep 41; the wiki's canonical Lauchengco statement on **product marketing in the AI era**. The foundational definition unchanged, the velocity flipped: **go-to-market is the new bottleneck**, not product building. **Ephemeral assets on a stable spine**; PMM as **judge and jury** of what gets elevated into a launch; **AEO / GEO** (Answer/Generative Engine Optimization) as a new positioning surface — *"models don't get technology wrong, so you must give them the technical depth"*; **AI slop** as polished-but-generic output and the *tone swizzle* that makes messaging ownable; the Gong-vs-Outreach differentiation case; *"great product marketing is what you take away, not what you add"* (Michelangelo's David). Backs [[product-marketing-by-stage]], [[messaging]], [[positioning]].

## Talks / sources referencing her
- [[2022-03-07-cagan-product-ops-dan-olsen]] — Cagan on why a strong PM needs a product-marketing partner; the "product-marketing rebranding" ops model.
- [[2023-05-17-cagan-10-misconceptions-startups-productx]] — *LOVED* named as the product-marketing book "already out" in the SVPG series.

## Notes
- **Not brand/field/performance marketing.** Product marketing is about market fit, positioning, go-to-market, and channel enablement — see [[product-marketing]] and the market side of [[time-to-money|product-market fit]].

## Talks & interviews ingested (2022 — the *LOVED* launch year)

Three sources from the four months around *LOVED*'s April 2022 release. The prepared material in the first two overlaps heavily; the value is in what each adds.

- [[2022-04-27-lauchengco-loved-lean-product-meetup]] — Lean Product Meetup (Dan Olsen), talk given 2022-04-19, **one week after the book shipped**. The canonical spoken walkthrough of the four fundamentals — [[pmm-ambassador|ambassador]] · [[pmm-strategist|strategist]] · [[pmm-storyteller|storyteller]] · [[pmm-evangelist|evangelist]] — with the go-to-market canvas walked row by row.
- [[2022-05-27-lauchengco-rethink-marketing-for-tech-products-red-id]] — RED-ID, Tel Aviv (May 2022). Same talk, **much better Q&A**: where product marketing should report, how PM and PMM actually split the work, and what to do when marketing says it cannot sell the roadmap. Also the clearest statement of the **when → why → what → how** inversion.
- [[2022-08-20-lauchengco-designing-products-that-are-loved-language-of-leadership]] — *Language of Leadership* #021 (Aug 2022). The conceptual-distinctions episode: **positioning ≠ messaging**, the **say-do gap** and its limit, and go-to-market shaping the product rather than only following it.

## The stories she reaches for

- **Microsoft Word, the halved release.** Universal edict to sim-ship with the next version of Windows; development time halved; feature count halved — in an era when the number of features on the box *was* the perceived value. A keystroke study of 200 users showed ~75% of all actions fell into four basic categories, and the enhancements were in those categories. Repositioned as *improving how people actually use a word processor*; became the best-reviewed and most commercially successful version of Word to that point. **The wiki's clearest evidence that positioning can carry a release the product alone would not.** They also deliberately ditched PowerPoint — at Microsoft — and drew the graph live on a whiteboard.
- **Word for Mac and word count.** A performance dog they could not fully fix. But *word count* is the one feature the press uses all day every day, and was their bellwether for performance — so they dedicated an engineer to it and made it the fastest word count on any platform. The press noticed and reported that performance had been addressed. Her canonical [[pmm-evangelist|evangelism]] example.
- **Looker vs. RJ Metrics.** Two analytics companies, same moment, same buyer. The short benefit-shaped line versus the specific technical one that tells you *why to believe it*. Mediocre outcome versus a multi-billion-dollar one — and RJ Metrics' CEO's own line about building a better horse-drawn carriage while Looker built the Model T.
- **[[workiva|Workiva]] / Web Filings multi-screen support.** The customer-success team's persistence against a product team's must-haves; the "nice to have" turned out to be the most-talked-about thing in the release. Her [[pmm-ambassador|ambassador]] example.
- **Splunk Trust.** Fezzes and capes for the deepest product nerds — scaled evangelism, because you cannot educate a technical market yourself.

## Notes

She is married to [[chris-jones|Chris Jones]], her fellow SVPG partner — she mentions it in the RED-ID Q&A while telling a story on him: he would say a release date was set because *the work was done*, and she would answer *"that's never a reason — where's the market thinking?"* A useful disclosure when both are cited on the same page, and a good illustration that market timing is a habit rather than an aptitude.

## Related
- [[svpg]], [[loved]], [[product-marketing]], [[marty-cagan]]
